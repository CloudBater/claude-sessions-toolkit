...
6. **`statusline.sh`** — Claude Code status line (bash). Renders `cwd · branch · model · session-name (size) · ctx:N% · 5h:N%↺ · 7d:N%↺ · vsc:N.NG N%`. Session file size sits immediately before live context usage so you read both as one "how much are we carrying" signal. After context come three optional segments, each green/yellow(≥70%)/red(≥90%): **`5h`/`7d`** rate-limit usage with reset time (from the `rate_limits.{five_hour,seven_day}` stdin fields — Pro/Max only, absent until the first API response, each window independently absent), and **`vsc`** VS Code memory (summed RSS of all VS Code processes as GB + % of total physical RAM; macOS-only, omitted when VS Code isn't running). Disk-only lookup (ignores `/rename`). Also writes a per-instance heartbeat (`~/.claude/runtime/instance-<session_id>.json`) so `/save`, `/read`, and `sr` can figure out which Claude terminal they're running in.
